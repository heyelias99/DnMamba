...
class ct_dataset(Dataset):
    def __init__(self, mode, load_mode, saved_path, test_patient, patch_n=None, patch_size=None, transform=None):
        ...
        if mode == 'train':
            ...
            # input_ = [f for f in input_path if not any(patient in f for patient in test_patient)]
            # target_ = [f for f in target_path if not any(patient in f for patient in test_patient)]
            ...
            # input_ = [f for f in input_path if any(patient in f for patient in test_patient)]
            # target_ = [f for f in target_path if any(patient in f for patient in test_patient)]
            if load_mode == 0:
                self.input_ = input_
                self.target_ = target_
            else:
